...
while (<LS>)
{
	...
	# First, slurp in the entire file
	@file = ();
	while (<IN>)
	{
		chomp;
	...
	while ($picked < $closecount{$symbol1.'|'.$symbol2})
	{
		...
		# Only keep cases where the closest rbp2 site is distant (>54 nt away)
		next if ($bindtype ne 'closest_is_distant');
