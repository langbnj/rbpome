...
%positive = ();
%negative = ();
$mainquery = Query("SELECT symbol1, symbol2, fdr FROM bioid");
startme("Getting BioID pairs from table 'bioid'");
while (($symbol1, $symbol2, $fdr) = Fetch($mainquery))
{
	# Skip anything that's not in our screen
	next if (!exists($screened{$symbol1}));
	next if (!exists($screened{$symbol2}));
...
state("Getting BioID validation ratios:");
...
while (($scoretype, $threshold, $source) = Fetch($typequery))
{
	...
	while (($symbol1, $symbol2, $eclip1, $eclip2) = Fetch($mainquery))
	{
		...
		{
			state("FALSE POSITIVE: $symbol1|$symbol2");
	...
	# Get TN/FN by cycling through the BioID-tested pairs that weren't screen hits
	foreach $pair (keys(%tested))
	{
		...
		if (!exists($screen_hit{$pair}))
		{
			...
			if ($positive == 1)
			{
				# state("FALSE NEGATIVE: $symbol1|$symbol2");
			...
			{
				# state("TRUE NEGATIVE: $symbol1|$symbol2");
