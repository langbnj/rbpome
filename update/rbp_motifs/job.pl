...
foreach $extend (1, 0)
{
	...
	# @fimosources = ('attract', 'dominguez', 'rnacompete');
	@fimosources = ('rnacompete', 'dominguez', 'attract');
...
# 	$table = "rbp_motifs$tmpextend_2\_grep_$type";
# 	# if (Exists($table))
# 	# {
...
#   `extend` tinyint(1) unsigned DEFAULT NULL,
...
# 		run("Get peak sequences", "get_peakseqs.pl grep $type $source$tmpextend");
#
# 		# All motifs (stored with a significant p-value)
# 		run("Grep for non-PWM motifs", "grep.pl $type $source$tmpextend");
#
# 		# Top motifs only (stored with a significant q-value)
# 		run("Grep for non-PWM motifs", "grep.pl $type $source -firstonly$tmpextend");
# 	}


	# Search for motif PWMs
	$table = "rbp_motifs$tmpextend_2\_fimo_$type";
	# if (Exists($table))
	# {
	# 	# Not clearing existing tables, keeping them & not rerunning
	# 	state(" >> Table $table already exists, skipping");
	...
	{
		$table = "rbp_motifs$tmpextend_2\_$method\_$type";
		# if (Exists($table))
		# {
		# 	# Not clearing existing tables, keeping them & not rerunning
		# 	state(" >> Table $table already exists, skipping");
	...
	{
		# Find motifs in eCLIP peak regions using MEME FIMO (only for RBPs with $source motifs)
		run("Get peak sequences", "get_peakseqs.pl fimo $type $source$tmpextend");

		# Run FIMO (p-value threshold)
		run("Run MEME FIMO", "fimo.pl $type $source 0.001$tmpextend");

		# # Run FIMO (q-value threshold)
		# run("Run MEME FIMO", "fimo.pl $type $source 0.05 -qvalue$tmpextend");
		...
		foreach $method (@fimomethods)
		{
			...
			run("Parse MEME FIMO output", "parse_fimo.pl $method $type $source 0.001$tmpextend");
			# run("Parse MEME FIMO output", "parse_fimo.pl $method $type $source 0.05 -qvalue$tmpextend");
		}
	}
