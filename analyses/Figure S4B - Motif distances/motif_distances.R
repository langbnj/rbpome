...
get_qfracs <- function() {
  ...
  my_qaccompanied <- foreach(i = 1:nrow(pairs_sample), .combine = 'bind_rows') %dopar% {
    ...
    qfrac <- tibble_row(symbol1=symbol1, symbol2=symbol2, pair=glue("{symbol1}|{symbol2}"), symbol2_motifs_accompanied=symbol2_motifs_accompanied, symbol2_motifs_total=symbol2_motifs_total, accompanied_fraction=symbol2_motifs_accompanied_fraction)
    qfrac
  }
  stopCluster(cl)
  write_rds(my_qaccompanied, glue("qaccompanied.rds"))
...
#   ggsave("output-motif-distances-all.pdf") +
#   ggsave("output-motif-distances-all.png")
...
qaccompanied %>%
  ...
  facet_wrap(~accnum, ncol=2, scales="free") +
  # facet_grid(rows = vars(motif_source), cols = vars(accnum), scales="free") +
  theme_minimal() +
  xlab("") +
  ylab("Number of RBP pairs") +
  ggsave("Figure R10A output-motif_distances-count1000-all.pdf", width=5, height=3, device=cairo_pdf) +
  ggsave("output-motif_distances-count1000-all.pdf", width=5, height=3, device=cairo_pdf) +
  ggsave("output-motif-distances-count1000-all.png", width=5, height=3)
...
#   xlab("") +
#   ylab("Number of RBP pairs") +
#   # ggsave("Figure R10B output-motif_distances-20percent-all.pdf", width=5, height=7, device=cairo_pdf) +
#   ggsave("output-motif_distances-20percent-all.pdf", width=5, height=7, device=cairo_pdf) +
#   ggsave("output-motif_distances-20percent-all.png", width=5, height=7)
# 
# # Plot 15%
# qaccompanied %>%
#   left_join(screenpairs) %>%
...
#   xlab("") +
#   ylab("Number of RBP pairs") +
#   # ggsave("Figure R10B output-motif_distances-15percent-all.pdf", width=5, height=7, device=cairo_pdf) +
#   ggsave("output-motif_distances-15percent-all.pdf", width=5, height=7, device=cairo_pdf) +
#   ggsave("output-motif_distances-15percent-all.png", width=5, height=7)
...
qaccompanied %>%
  ...
  geom_bar() +
  # facet_wrap(~accfrac, scales="free_y") +
  facet_wrap(~accfrac, ncol=2, scales="free") +
  # facet_grid(rows = vars(motif_source), cols = vars(accfrac), scales="free") +
  theme_minimal() +
  xlab("") +
  ylab("Number of RBP pairs") +
  ggsave("Figure R10B output-motif_distances-10percent-all.pdf", width=5 , height=3, device=cairo_pdf) +
  ggsave("output-motif_distances-10percent-all.pdf", width=5, height=3, device=cairo_pdf) +
  ggsave("output-motif_distances-10percent-all.png", width=5, height=3)
...
qaccompanied %>%
  ...
  arrange(-accompanied_fraction) %>%
  print(n=100) %>%
  write_tsv("output-table-S10-allsources.tsv")
